...
pub struct PageTableImpl {
    ...
    root_frame: Frame,//作为根的三级页表所在的物理页帧
    entry: Option<PageEntry>,//在操作过程中临时使用
}

impl PageTableImpl {
    //新建空页表
    pub fn new_bare() -> Self {
        //分配物理页帧并获取物理地址，作为根的三级页表就放在这个物理页帧中
        let frame = alloc_frame().expect("alloc_frame failed!");
        let paddr = frame.start_address().as_usize();
        //利用access_pa_via_va访问物理页帧并进行页表初始化
